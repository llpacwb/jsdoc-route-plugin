...
exports.options = {
  ...
  onTagged: function(doclet, tag) {
    if (!doclet.queryparams) {
      doclet.queryparams = [];
    }

    doclet.queryparams.push({
      'name': tag.value.name,
...
exports.newDocletHandler = function(e) {
  const parameters = e.doclet.queryparams;
  if (parameters) {
    const table = tableBuilder.build('Query Parameters', parameters);

    e.doclet.description = `<div class="route-description">${e.doclet.description}</div>
                            ${table}`;
  }
}
